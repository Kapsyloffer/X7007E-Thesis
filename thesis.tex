...
The Mixed-Model Assembly Line (MMAL) problem is formally classified as NP-Hard. Traditionally, these optimization problems are addressed using heuristic or meta-heuristic algorithms. %See \cite{ch3}
...
To address the limitations of fixed vocabulary models in our combinatorial problem, we implemented a Pointer Network based on the architecture proposed by Vinyals et al. (2015) \cite{ref7}. Unlike the Transformer or Seq2Seq models which generate outputs based on a predefined vocabulary, the Pointer Network uses an attention mechanism to select discrete elements directly from the input sequence. This makes it inherently suitable for problems such as the product sequencing problem, where the output is a permutation of the input stations.

\paragraph{1. Feature Projection}
The input to the model consists of a sequence of $N$ stations, where each station is represented by a continuous feature vector $x_i \in \mathbb{R}^{input\_dim}$ (representing station size and offset). Since these inputs are not discrete tokens, we first project them into a latent feature space using a learnable projection layer:
\begin{equation}
    e_i = \text{LayerNorm}(\tanh(W_{proj} x_i + b_{proj}))
\end{equation}
where $e_i \in \mathbb{R}^{d_{model}}$ serves as the embedding for the $i$-th station.

\paragraph{2. Bidirectional Encoder}
To capture the dependencies between stations across the entire production sequence, the embedded inputs are passed through a Bidirectional Long Short-Term Memory (Bi-LSTM) network. This processes the sequence in both forward and backward directions:
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
The final encoder output for each station $u_i$ is the concatenation of the forward and backward hidden states, $u_i = [\vec{h}_i; \overleftarrow{h}_i]$, doubling the hidden dimension to capture rich contextual information.

\paragraph{3. Decoder and Initialization}
The decoder is a bidirectional LSTM Cell that constructs the output sequence step-by-step. To ensure the decoder starts with a comprehensive understanding of the input, its initial hidden state $d_0$ and cell state $c_0$ are initialized via a "bridge" layer. This layer transforms the final hidden states of the encoder (a global summary of the input sequence) into the dimensionality required by the decoder:
\begin{equation}
    d_0 = \tanh(W_{bridge} [ \vec{h}_N; \overleftarrow{h}_0 ])
\end{equation}

\paragraph{4. Attention and Pointer Mechanism}
At each time step $t$, the decoder maintains a hidden state $d_t$. The core of the model is the pointer mechanism, which calculates a probability distribution over the input elements to decide which station to place next. 

Our implementation utilizes a robust dual-attention scoring mechanism. We compute two separate attention scores, a "glimpse" score and a "pointer" score, which are summed to produce the final logits. For an input index $j$ and decoder step $t$, the unnormalized score $s_{t,j}$ is calculated as:
\begin{equation}
    s_{t,j} = v^T \tanh(W_{ref} u_j + W_{query} d_t)
\end{equation}
This score represents the compatibility between the current decoding state $d_t$ and the input station $u_j$. The final probability distribution is obtained via softmax:
\begin{equation}
    p(j | \pi_{<t}, \mathcal{P}) = \text{softmax}(s_{t,j})
\end{equation}
The index with the highest probability is selected as the next element in the sequence.

\paragraph{5. Masking and Constraints}
A critical constraint in permutation problems is that each input item must appear exactly once in the output. To enforce this, we maintain a dynamic mask of visited indices. Once an index $k$ is selected at step $t$, its corresponding attention score is set to $-\infty$ for all future steps $t' > t$:
\begin{equation}
    s_{t', k} = -\infty \quad \forall t' > t
\end{equation}
This effectively zeroes out the probability of re-selecting the same station, guaranteeing that the model produces a valid permutation of the assembly line.

\paragraph{6. Input Feeding}
Unlike standard language models that feed the generated token back into the decoder, the Pointer Network feeds the \textit{encoder output} of the selected station. If index $k$ is chosen at step $t$, the input to the decoder at step $t+1$ is the vector $u_k$. This allows the decoder to track which station was just placed and decide what should optimally follow it.
\subsection{The Sequence-to-Sequence Model}

\subsection{The Dataset}
To adapt the Transformer architecture, originally designed for discrete natural language tokens, to the Mixed Model Assembly Line (MMAL) problem, we implemented a custom data handling pipeline. The \texttt{Dataset} class inherits from \texttt{torch.utils.data.Dataset} and is responsible for parsing the raw problem instances into feature vectors suitable for combinatorial optimization.

\paragraph{Data Representation}
As stated previously, unlike standard NLP tasks where the input consists of discrete token IDs mapped to a vocabulary, the input for the assembly line problem consists of continuous variables representing station attributes. The raw data is ingested from JSON files. For each problem instance, we extract $N$ stations.

Each station $i$ is represented as a feature vector $\mathbf{x}_i \in \mathbb{R}^2$, defined as:
\begin{equation}
    \mathbf{x}_i = [\text{data}_i, \text{offset}_i]
\end{equation}
where $\text{data}_i$ represents the workload or processing attribute, and $\text{offset}_i$ represents the physical or temporal constraint of the station. Consequently, a single input sample is a tensor of shape $(N, 2)$, effectively treating the list of stations as a "sentence" of length $N$ with an embedding dimension of 2.

\paragraph{Target Permutations}
The objective of the model is to learn the optimal sequencing of these stations. Therefore, the target output is not a sequence of foreign words, but a \textbf{permutation} of the input indices.
For a sequence of length $N$, the ground truth target is generated as an index tensor $y \in \{0, \dots, N-1\}^N$. In this implementation, the targets represent the correct positional indices required to reconstruct the optimal assembly sequence.

\paragraph{Dataset Partitioning}
To ensure rigorous evaluation, the dataset is loaded and immediately shuffled and partitioned. We employ a hold-out validation strategy defined by the parameter \texttt{train\_frac} (set to 0.8):
\begin{itemize}
    \item \textbf{Training Set (80\%):} Used for gradient updates via backpropagation.
    \item \textbf{Validation Set (20\%):} Used to monitor convergence and check for overfitting during the training loop.
\end{itemize}

\paragraph{Batch Collation}
Transformers process data in parallel batches. Since assembly line problems may have varying numbers of stations (sequence lengths), we implement a custom \texttt{collate\_fn}. This function receives a list of tuples $(\mathbf{x}, y)$ and stacks them into batched tensors:
\begin{itemize}
    \item \textbf{Samples Batch:} Shape $(B, N, 2)$, where $B$ is the batch size.
    \item \textbf{Targets Batch:} Shape $(B, N)$, containing the ground truth integer indices.
\end{itemize}
This structure allows the model to predict the probability distribution over input indices at every step of the decoding process.
...
\subsection{Unhooking the decoder}
...
\section{Dataset}
We obtained an official dataset from a production run. However, the raw data contained several inconsistencies that required resolution before it could be used for training.

\paragraph{Data Inconsistencies}
An initial analysis of the raw production data revealed approximately 2,000 overlaps if no offsets were applied. After algorithmically fitting the data to respect station constraints, the number of overlaps was reduced to 300. 

 Given that this dataset represents a completed production run which ostensibly proceeded without line stoppages, the remaining 300 overlaps imply either unrecorded manual interventions by operators or minor inaccuracies in the logged timestamps. [cite\_start]For the purpose of this study, these 300 overlaps are treated as the "noise floor" of the dataset[cite: 1, 22].

\section{Evaluation Metrics}
To objectively measure the performance of the model, we define the \textbf{Overlap Reduction Rate (ORR)}. An "overlap" occurs when the processing time of an item at station $S_n$ combined with its offset exceeds the available time window, causing a collision with the subsequent item.

The metric is calculated as:
\begin{equation}
    \text{ORR} = \frac{O_{initial} - O_{predicted}}{O_{initial}} \times 100\%
\end{equation}
where $O_{initial}$ is the total overlap duration (in cmin) of the shuffled input sequence, and $O_{predicted}$ is the total overlap duration of the sequence permuted by the model.

\section{Quantitative Results}
\subsection{Convergence and Capacity Analysis (Overfitting)}
To verify the model architecture's capacity to learn the complex constraints of the Mixed-Model Assembly Line, we first conducted a "sanity check" by overfitting the model on a small, fixed subset of the data. The objective was to determine if the Pointer Network could theoretically solve the sequencing puzzle given sufficient exposure.

As shown in Table \ref{tab:overfit_results}, the model achieved a **90\% reduction** in overlaps. This confirms that the vector representation of stations (size and offset) contains sufficient information for the model to identify and resolve scheduling conflicts, effectively validating the architecture described in Chapter 4.

\subsection{Generalization Performance}
Following the capacity analysis, the model was evaluated on a held-out validation set to assess its generalization capabilities. This test simulates a real-world scenario where the model encounters unseen production orders.

On the validation set, the model achieved a **25\% reduction** in overlaps compared to the randomized baseline. While significantly lower than the overfitting result—a discrepancy attributed to the limited size of the historical dataset—this 25\% improvement indicates that the model successfully learned to identify the most critical conflict patterns.

\section{Runtime Analysis}
The inference time for the Pointer Network is linear $O(N)$ with respect to the sequence length, taking approximately 0.4 seconds to sequence a batch of 100 items. This stands in contrast to the heuristic approach, which scales quadratically or worse depending on the depth of the search.
...
\textit{Programming PyTorch for Deep Learning, ISBN: I pointer deep learning},
O'Reilly Media, 2019.
\bibitem{ref5} 
