...
\begin{table}[htbp]
    ...
    \end{tabular}
\end{table}
\subsection{Abstracting the Data}
For ease of training, the training data has been essentially stripped down to its three most relevant components, \textit{object\_id}, \textit{[data]}, \textit{[offsets]}, where object\_id is the identifier, and data \& offsets are arrays containing the time requirements for each station labeled $s1$, $s2$, \dots :
