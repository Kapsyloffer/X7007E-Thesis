...
In recent years, Deep Reinforcement Learning (DRL) has become a dominant approach for solving dynamic scheduling problems. \cite{ref11} Unlike supervised methods, DRL agents learn through trial-and-error interaction with a simulation environment, aiming to maximize a cumulative reward signal.
...
The architecture utilized in this thesis is heavily based on the canonical Transformer model proposed by Vaswani et al. \cite{ref5}, with significant modifications tailored specifically to the Mixed-Model Assembly Line (MMAL) problem, and adapted to function as a cost estimator for the assembly line. For one, the decoder has been decoupled to yield better results (see \ref{subp:decoupling}), and instead of processing text, the model has been fitted with a custom embedding layer which is tasked with reading the size of the work-load within a station, and find a better fit.

\subsubsection{Simulated Annealing} While the Transformer predicts the local impact of individual items, the global sequence scheduling is addressed using Simulated Annealing. This method explores the solution space of item permutations, using a cost matrix derived from the Transformer's drift predictions. By allowing for probabilistic acceptance of sub-optimal transitions Simulated Annealing avoids local optima that typically constrain greedy algorithms, resulting in a more stable assembly line schedule. 


\subsubsection{Unhooking the decoder}
During preliminary feasibility testing, empirical results indicated that a standard Sequence-to-Sequence (Encoder-Decoder) framework was ill-suited for the regression nature of the cost estimation task. Consequently, the architecture was simplified to an Encoder-only configuration, conceptually similar to BERT. This modification eliminates the autoregressive generation step, allowing the model to focus entirely on learning bidirectional contextual representations of the station workload.
...
\subsubsection{Simulated Annealing} Similar to the Transformer model this too uses Simulated Annealing.
...
\bibitem{ref11} T. Mitchell, \textit{Machine Learning}, ISBN, publisher, 1997. (FIXME)
