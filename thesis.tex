...
\subparagraph{Hybrid Continuous-Discrete Embedding}
Standard NLP models map a finite set of words to static vectors. Since product variations (specifically size and weight) are continuous rather than discrete, a traditional vocabulary approach would require an infinite vocabulary size to capture every possible variation.

To resolve this, the implementation utilizes a hybrid \textit{ObjectEmbedding} layer. The discrete station identifier is mapped via a standard lookup table, while the continuous size value is projected through a linear layer. These two feature sets are concatenated to form the input vector:
$$
...
\subparagraph{Decoupling Prediction from Sequencing}
To strictly prevent the repetition of tokens common in generative language models, the sequence generation is decoupled from the neural network entirely.
...
$$
By using the Transformer solely for high-fidelity feature extraction and delegating the sorting logic to a mathematical heuristic, the system ensures that the output is always a valid permutation of the inputwhich will guarantuee that no items are duplicated or omitted.
...
\subsection{The Transformer Model}
TODO
\paragraph{The Greedy Algorithm} The Transformer model utlizes a Greedy Algorithm for better fitting objects
\subsection{The Pointer Network Model}
TODO
\subsection{The Dataset}
TODO
\subsection{The Training Loop}
TODO, (see \ref{chap:workarounds})
...
\begin{table}[htbp]
    \centering
    \caption{Comparison model performance in overlap reduction for $S$ = $e^3$ (TODO)}
    \label{tab:overlap_results}
    \begin{tabular}{l c c}
        \toprule
        \textbf{Training Size} & \textbf{Overlap Ratio ($R$)} & \textbf{} \\
        \midrule
        $1e^3$ & 0.45 & 110 \\
        $1e^6$ & 0.62 & 125 \\
        $1e^9$ & 0.78 & 140 \\
        $1e^{12}$ & \textbf{0.85} & 138 \\
        \bottomrule
    \end{tabular}
    \centering
    \caption{Comparison model performance in overlap reduction for $S$ = $e^6$ (TODO)}
    \label{tab:overlap_results}
    \begin{tabular}{l c c}
        \toprule
        \textbf{Training Size} & \textbf{Overlap Ratio ($R$)} & \textbf{} \\
    ...
    \end{tabular}
    \centering
    \caption{Comparison model performance in overlap reduction for $S$ = $e^9$ (TODO)}
    \label{tab:overlap_results}
    \begin{tabular}{l c c}
        \toprule
        \textbf{Training Size} & \textbf{Overlap Ratio ($R$)} & \textbf{} \\
...
\section{The Pointer Network}
\subsection{Adding the offset handler}
...
To objectively measure the performance of the model, we monitor performance of the models in overlap reduction (see \ref{chap:overlap}) using a simple ratio. The ratio is:
\begin{equation}
