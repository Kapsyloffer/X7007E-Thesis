...
\subparagraph{Continuous-Discrete Embedding}
...
The positioning of an object within a station is determined by its \textit{offset} relative to the \textbf{Takt} start. The script employs a centering logic to ensure that tasks are placed as close to the middle of the available window as possible, minimizing the risk of drift violations.

The calculation of the left-hand boundary offset, $O_{left}$, is defined as:
\begin{equation}
    O_{left} = L_{slot} - (T \times C_{min})
\end{equation}
Where $L_{slot}$ is the leftmost available time slot and $T \times C_{min}$ represents the theoretical start of the takt.

To determine the ideal centered position, the script first calculates the remaining buffer on the right, $O_{right}$:
\begin{equation}
    O_{right} = C_{min} + D_{area} - O_{left} - S_{new}
\end{equation}
Where $D_{area}$ is the drift area and $S_{new}$ is the duration of the task at the station. The final offset, $O$, is then calculated by attempting to center the task within these boundaries:
\begin{equation}
    O_{raw} = O_{left} + \frac{O_{right}}{2}
\end{equation}

Finally, to ensure the object remains within physical station boundaries, the raw offset is clamped within the permissible drift range so that 
\begin{equation}
    Offset = -D \leq 0 \leq (T + D - S)
...
\paragraph{/run/\{model\_type\}/$int:n$/}
...
\subsection{Adding the offset handler}
    While the Transformer model used a simulated annealing algorithm for adjusting offsets on runtime, the same idea was applied to the pointer network (See \ref{sec:trans})
...
\section{Interpretation of Results}
    I think I did good. (FIXME)
\section{Limitations}
    The data was bad, we're cooked. (FIXME)
\section{Future Work}
    Do again with good data and a million GPUs. (FIXME)
